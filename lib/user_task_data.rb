class UserTaskData
  def initialize args = {}
    ...
    @fission_log = FissionLog.where(user_id: args[:user_id], task_id: args[:task_id]).first
  end
  ...
  # 金币数
  def coin_logs
    CoinLog.where(channel: 'fission', model_id: @fission_log.subtree_ids).where(created_at: @date_from..@date_to)
  end

  # 分享数
  def share_logs
    ShareLog.where(fission_log_id: @fission_log.subtree_ids).where(created_at: @date_from..@date_to)
  end

  # 查看次数
  def view_logs
    ViewLog.where(fission_log_id: @fission_log.subtree_ids).where(created_at: @date_from..@date_to)
  end
